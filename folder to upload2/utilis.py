...
def splitBoxes(img,questionsperc,choices):
    rows = np.vsplit(img,questionsperc)#number of questions 25 FOR MY OWN PROGRAM
    boxes=[]
    for r in rows:
        cols= np.hsplit(r,choices)#number of options 4 FOR MY OWN PROGRAM
        for box in cols:
            boxes.append(box)
    return boxes
